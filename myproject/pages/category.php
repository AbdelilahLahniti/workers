...
$getSpecialty = mysqli_query($dataBase, "SELECT DISTINCT specialty FROM worker");
if (isset($_GET['city'])) {
    $city = $_GET['city'];

    $getData = mysqli_query($dataBase, "SELECT * FROM worker WHERE city='$city' && specialty='$specialty' ");

    $specialtyCity = mysqli_query($dataBase, "SELECT specialty FROM worker WHERE city='$city'");
} else {
    $getData = mysqli_query($dataBase, "SELECT * FROM worker WHERE specialty='$specialty' ");
}
...
<body>
    ...
    <div class="container-fluid row">
                    ...
                    <ul class="list-unstyled">
                        ...
                        <li class="nav-item dropdown py-2">
                            ...
                            <ul class="dropdown-menu position-relative" aria-labelledby="navbarDropdown">
                                ...
                                ?>
                                    <li><a class="dropdown-item text-uppercase" id='city' href="workerCity.php?city=<?php echo $row['city'] ?>"> <?php echo $row['city'] ?></a></li>
                                <?php } ?>
                            </ul>
                        </li>
                        <li class="nav-item dropdown py-2">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                                <?php
                                if (isset($city)) {
                                    while ($row = mysqli_fetch_assoc($specialtyCity)) {
                                ?>
                                        <li><a class="dropdown-item text-uppercase" href="<?php echo $_SERVER['PHP_SELF']; ?>?specialty=<?php echo $row['specialty'] ?>&&city=<?php echo $city ?>"> <?php echo $row['specialty'] ?></a></li>
                                    <?php } ?>
                                    <?php } else {
        ...
        <div class="container col-10 workers pt-2 pt-lg-0">
            <h1 class="text-center my-5 title"><?php echo $specialty ?></h1>
